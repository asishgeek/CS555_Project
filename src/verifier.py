...
def printUsage():
        print "Usage %s <CommonInputFile> [<Transcript location>]"%sys.argv[0]
...
def verifier():
        ...
        for iteration in range(0,iterCount):
                ...
                fname = "%s/transcript_verifier_iter_%d_%s.txt"%(transcriptLocation, iteration, uid)
                fp = open(fname, 'w')
